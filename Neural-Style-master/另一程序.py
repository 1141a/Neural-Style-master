...
MEAN_VALUES = np.array([123.68, 116.779, 103.939]).reshape((1,1,1,3)) #normalization中得到
...
def generate_noise_image(content_image, noise_ratio = NOISE_RATIO):
    ...
    # of the values
    input_image = noise_image * noise_ratio + content_image * (1 - noise_ratio) #TODO 注意比较噪声图
    return input_image
...
def load_vgg_model(path):
    ...
    def _weights(layer, expected_layer_name):
        ...
        """
        W = vgg_layers[0][layer][0][0][4]
        b = vgg_layers[0][layer][0][0][3]
...
def style_loss_func(sess, model):
    ...
    def _gram_matrix(F, N, M):
        ...
        """
        Ft = tf.reshape(F, [M, N])
        return tf.matmul(tf.transpose(Ft), Ft)#TODO 注意区别ATA
